...
if ENV['SENTRY_DSN'] && !ENV['SENTRY_DSN'].empty?
  ...
  # Enable Sentry when DSN is provided
  # Include Render's 'deployment' environment
  if ENV['SENTRY_DSN']
    config.enabled_environments = [ENV.fetch('RACK_ENV', 'development')]
  else
  ...
  # Server name
  config.server_name = ENV.fetch('RENDER_SERVICE_NAME', 'actorsync')
...
def filter_sensitive_headers(headers)
  ...
  headers.each do |key, value|
    if sensitive_headers.any? { |sensitive| key.to_s.downcase.include?(sensitive) }
